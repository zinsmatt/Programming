...
int main()
{
	...
	F(t, tt)
	{
		...
		cin >> n;
		vector<int> a(n);
		vector<int> occ_min(n+1, -1);
		vector<int> occ_max(n+1, -1);
		F(i, n) {
			cin >> a[i];
			if (occ_min[a[i]] == -1 || occ_min[a[i]] > i)
				occ_min[a[i]] = i;
			if (occ_max[a[i]] == -1 || occ_max[a[i]] < i)
				occ_max[a[i]] = i;
		}
		bool  ok = false;
		F(i, n+1)
		{
			if (occ_min[i] != -1 && occ_max[i] != -1 && occ_max[i] - occ_min[i] > 1) {
