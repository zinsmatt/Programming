...
int main()
{
	...
	F(t, tt)
	{
        ...
        F (i, n) {
            ...
            {
                if (j == i) { // case 2
                    bool r = (pref[n-1]^pref[i]) ==  pref[i];
                    ok |= r;
                    if (r){
                        // cout << "ij = " <<i << " " <<  j << " => " << pref[i] << "   " <<  (pref[n-1] ^ pref[i]) << "\n";
                        break;
                    }
                    
                } else {    // case 3
                    bool r = (pref[i] == (pref[j]^pref[i])) && (pref[i] == pref[n-1]^pref[j]);
                    ok |= r;
                    if (r){
                        // cout << "ij = " <<i << " " <<  j << " => " << pref[i] << " " <<  (pref[j]^pref[i]) << "  " << (pref[n-1]^pref[j]) << "\n";
                        break;
                    }
                }
            }
