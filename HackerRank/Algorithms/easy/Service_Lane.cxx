...
vector<int> serviceLane(int n, vector<vector<int>> cases, vector<int> const& width) {
    vector<int> res(cases.size());
    for (int i = 0; i < cases.size(); ++i)
    {
        int min = *std::min_element(width.begin()+cases[i][0], width.begin()+cases[i][1]+1);
        res[i] = min;
    }
    return res;
}
